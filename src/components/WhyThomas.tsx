...
const reasons = [
  ...
  },
  {
    ...
    body: 'Need a bigger repair? Ask about flexible financing options when you call — we work with your budget.',
    wide: true,
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    <section className="bg-navy py-14 md:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-[340px_1fr] gap-6 items-stretch">
          ...
          {/* Owner photo — h-full fills to match bento height */}
          <div className="relative rounded-2xl overflow-hidden shadow-2xl min-h-[400px] h-full">
            <Image
            ...
            />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/40 to-transparent px-5 pt-12 pb-5">
              ...
              <p className="text-white/60 text-xs mt-2 font-semibold tracking-wide uppercase">Will Thomas, Owner</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* 3-col bento: row1=[wide, narrow], row2=[narrow×3], row3=[narrow, wide] */}
            <div className="grid grid-cols-3 gap-3 flex-1">
              ...
              {reasons.map((r, i) => {
                ...
                return (
                  <div
                    key={r.title}
                    className={`group bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-2xl p-4 transition-all duration-200 flex gap-3 ${
                      isWide ? 'col-span-2 items-start' : 'col-span-1 flex-col'
                    }`}
                  >
                    <div className={`shrink-0 rounded-xl bg-brand-red/20 group-hover:bg-brand-red/35 flex items-center justify-center text-white transition-colors duration-200 ${
                      isWide ? 'w-10 h-10 mt-0.5' : 'w-9 h-9'
                    }`}>
                      {r.icon}
                    </div>
                    <div>
                      <h3 className="font-semibold text-white text-sm leading-snug mb-1">{r.title}</h3>
